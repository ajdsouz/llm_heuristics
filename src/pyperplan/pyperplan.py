...
SEARCHES = {
    ...
    "gbfs_early_goal_test": search.gbfs_early_goal_test,
    "gbfs_early_goal_test_int": search.gbfs_early_goal_test_int,
...
def search_plan(
    ...
    logging.info("Total time: {:.2f}".format(time.process_time() - total_start_time))
    return solution
...
def main():
    ...
    argparser.add_argument(
        "-H",
        "--heuristic",
        help=f"Select a heuristic. Built-in heuristics are {', '.join(HEURISTICS.keys())}.",
        default="hff",
    )
    argparser.add_argument(
        "-s",
    ...
    if Path(args.heuristic).is_file():
        with open(args.heuristic, "r") as f:
            heuristic = f.read()
            module = tools.import_python_file(args.heuristic)
            heuristic = [
                    getattr(module, cls)
                    for cls in dir(module)
                    if cls.endswith("Heuristic")
    ...
    if args.search in ["bfs", "ids", "sat"]:
        if args.heuristic != "hff":
            logging.warning(f"Search {args.search} does not use a heuristic")
        heuristic = None
